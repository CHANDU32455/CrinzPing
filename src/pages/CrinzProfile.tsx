...
import { UserMemes } from '../components/profile/ProfilePosts';
import UserReels from "../components/profile/ProfileReels";
import { About } from '../components/profile/ProfileAbout';
import { Settings } from '../components/profile/ProfileSettings';
import { FloatingActionButton } from '../components/shared/ActionButtons';
import { ProfileSEO } from '../components/shared/seoContent';
...
const CrinzProfile: React.FC = () => {
    ...
    // Main tabs state (Profile, About, Settings) - Initialize from URL
    const [mainActiveTab, setMainActiveTab] = useState<'profile' | 'about' | 'settings'>(() => {
        const tab = searchParams.get('tab');
        if (tab === 'about' || tab === 'settings') return tab;
        return 'profile';
    });
    // Inner tabs state (Messages, Posts, Reels) - Initialize from URL
    const [innerActiveTab, setInnerActiveTab] = useState<'messages' | 'posts' | 'reels'>(() => {
        const inner = searchParams.get('inner');
        if (inner === 'posts' || inner === 'reels') return inner;
        return 'messages';
    });
    ...
    // Sync tabs with URL changes (browser back/forward)
    useEffect(() => {
        const tab = searchParams.get('tab');
        const inner = searchParams.get('inner');
        ...
        if (tab === 'about' || tab === 'settings') {
            ...
            setMainActiveTab('profile');
        }

        if (inner === 'posts' || inner === 'reels') {
            setInnerActiveTab(inner);
        } else if (inner === 'messages' || !inner) {
            setInnerActiveTab('messages');
        }
    }, [searchParams]);
    ...
    const handleMainTabChange = (tab: string) => {
        if (tab === 'profile' || tab === 'about' || tab === 'settings') {
            ...
            // Keep inner tab param if on profile tab
            if (tab === 'profile' && innerActiveTab !== 'messages') {
                newParams.set('inner', innerActiveTab);
            } else {
                newParams.delete('inner');
            }
            navigate({ search: newParams.toString() }, { replace: true });
        }
    };

    // Inner tab change - Update URL
    const handleInnerTabChange = (tab: string) => {
        if (tab === 'messages' || tab === 'posts' || tab === 'reels') {
            setInnerActiveTab(tab);
            // Update URL with new inner tab
            const newParams = new URLSearchParams(searchParams);
            newParams.set('tab', 'profile'); // Ensure we're on profile tab
            if (tab !== 'messages') {
                newParams.set('inner', tab);
            } else {
                newParams.delete('inner'); // Default is messages, no need in URL
            }
            navigate({ search: newParams.toString() }, { replace: true });
        }
    };
    ...
    // Post click
    const handlePostClick = (post: CrinzMessage) => {
        ...
        return (
            <div className="min-h-screen bg-black flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-purple-300">{getRoast.loading('auth')}</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-black flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-purple-300 mb-4">Back to prove you actually have friends?</p>
                    <button
                        onClick={() => auth.signinRedirect()}
                        className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors"
                    >
                        {getRoast.action('signIn')}
                    </button>
                </div>
            </div>
        );
        ...
        return (
            <div className="min-h-screen bg-black flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-purple-300">This person blocked you... just kidding, they don't exist 👻</p>
                </div>
            </div>
        );
    }
    ...
    if (isOwnProfile && !userDetails && !shouldRedirectToSetup) {
        return (
            <div className="min-h-screen bg-black flex items-center justify-center">
                <div className="text-center">
                    <div className="text-6xl mb-4">💥</div>
                    <h1 className="text-2xl text-white font-bold mb-2">Oops! Something Broke</h1>
                    <p className="text-purple-300 mb-4">{getRoast.error('general')}</p>
                    <button
    ...
    return (
        <>
            {/* Dynamic Profile SEO - Uses user's profile pic for social sharing */}
            <ProfileSEO
                                    ...
                                    <div className="space-y-3">
                                        {loadingCrinz ? (
                                            <div className="text-center py-8">
                                                ...
                                                <p className="text-gray-400 text-sm">Loading your cringe content...</p>
                                            </div>
                                        ) : !crinzMessages || crinzMessages.length === 0 ? (
                                            <div className="text-center py-8">
                                                <div className="text-4xl mb-3">💀</div>
                                                <p className="text-gray-400">{getRoast.empty('messages')}</p>
                                                {isOwnProfile && (
                                                    <button
